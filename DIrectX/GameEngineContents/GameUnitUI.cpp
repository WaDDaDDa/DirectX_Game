...
{
	Transform.AddLocalPosition({0.0f, 0.0f, 200.0f});
	// 궁극기 온/오프 테두리
	...
	Renderer->Transform.AddLocalPosition({ 0.0f, 0.0f, -static_cast<float>(ContentsOrder::HPLayer1) });

	// 바탕
	Renderer5 = CreateComponent<GameEngineSpriteRenderer>(ContentsOrder::HPLayer2);
	...
	Renderer5->Transform.AddLocalPosition({ 0.0f, 0.0f, -static_cast<float>(ContentsOrder::HPLayer2) });

	// mp
	Renderer2 = CreateComponent<GameEngineSpriteRenderer>(ContentsOrder::HPLayer3);
	...
	Renderer2->Transform.AddLocalPosition({ 0.0f, 0.0f, -static_cast<float>(ContentsOrder::HPLayer3) });

	// hp
	Renderer3 = CreateComponent<GameEngineSpriteRenderer>(ContentsOrder::HPLayer3);
	...
	Renderer3->Transform.AddLocalPosition({ 0.0f, 0.0f, -static_cast<float>(ContentsOrder::HPLayer3) });
...
{
	float UIZ = Transform.GetWorldPosition().Z;
	float4 UnitPos = Unit->Transform.GetWorldPosition();
	Transform.SetWorldPosition({ UnitPos.X, UnitPos.Y, UIZ });
