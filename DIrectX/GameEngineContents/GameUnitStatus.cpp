...
GameUnitStatus GameUnitStatus::KnightStat =
{
    ...
    4.2f,                   //_SkillCooltime
    20.0f,                   //_UltCooltime
    std::string_view("모든 적 유닛을 일정 시간동안 도발하여 해당 적의 공격 \n대상을 자신으로 만듭니다."),
    std::string_view("일정 시간동안 아군 모두의 방어력을 자신의 방어력 만큼 \n상승시킵니다."),
    std::string_view("기사")
};
...
GameUnitStatus GameUnitStatus::MonkStat
{
    ...
    std::string_view("주변 일정 범위 아군에게 무적효과가 있는 보호막을 \n부여합니다."),
    std::string_view("몽크")
};
...
GameUnitStatus GameUnitStatus::PythonessStat
{
    ...
    6.5f,                   //_SkillCooltime
    24.0f ,                  //_UltCooltime
    std::string_view("아군 한명의 체력을 회복하고 해당 아군 주변에 범위에 피해를 \n주는 폭발을 생성합니다."),
    std::string_view("아군을 회복시키거나 적군을 공격하는 영혼을 방출하는 문을 \n소환합니다."),
    std::string_view("무녀")
};
...
GameUnitStatus GameUnitStatus::PyromancerStat
{
    ...
    std::string_view("주변 일정 범위에 지속적으로 공격을 가하는 영역을 \n생성합니다."),
...
GameUnitStatus GameUnitStatus::PriestStat
{
    ...
    7.0f,                   //_SkillCooltime
    30.0f   ,                //_UltCooltime
    std::string_view("일반공격이 아군을 회복하며 스킬 사용시 보다 더 많은 \n회복을 합니다."),
    std::string_view("주변 일정 범위에 지속적으로 체력을 회복시키는 영역을 \n생성합니다."),
    std::string_view("성직자")
};
...
GameUnitStatus GameUnitStatus::SwordmanStat
{
    ...
    std::string_view("일직선으로 빠르게 이동하며 경로상에 있는 모든 적을 \n베어버립니다."),
    std::string_view("검사")
};
