...
void GameEngineTileMap::Render(GameEngineCamera* _Camera, float _Delta)
{
	...
	int StartX = (ScreenLeftTop.iX() / TileData.TileScale.iX()) - 1;
	int StartY = (ScreenLeftTop.iY() / TileData.TileScale.iY()) - 1;
	int EndX = StartX + (WindowScale.iX() / TileData.TileScale.iX()) + 1;
	int EndY = StartY + (WindowScale.iY() / TileData.TileScale.iY()) + 1;
